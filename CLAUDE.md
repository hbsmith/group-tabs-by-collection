...
**Contiguity invariant:** each group must be a physically contiguous run of tabs with its chip anchored to the leftmost member. `_reflowTabs(window)` enforces this — it reorders all grouped tabs into `st.groups`/`tabIds` order after the library tab — and is called (with the observer disconnected) after every mutation and in the observer's debounced callback, so a stray native tab-drag that splits a group self-heals on the next tick. Chip placement in `_renderGroupChips` independently anchors to the physically-leftmost open member, not `tabIds[0]`. Don't add a code path that moves grouped tabs without going through `_reflowTabs`.

**Manual overrides** (right-click "Move to group"/"New group…", or drag a tab onto a chip or onto any tab in the target group): recorded in `st.overrides` and take precedence over collection-based assignment everywhere. Drag state is tracked via the module-level `_draggingTabId` (set on a delegated `dragstart` that only fires for reader/note tabs), not `dataTransfer` type checks, which are unreliable in Gecko during `dragover`. All reader tabs are made `draggable` in `_renderGroupChips`; the tab-region drop target is a delegated `dragover`/`drop` pair in `_setupTabBarObserver`.
