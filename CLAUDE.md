...
**Contiguity invariant:** each group must be a physically contiguous run of tabs with its chip anchored to the leftmost member. `_reflowTabs(window)` enforces this *without fighting the user*: it re-derives each group's member order from the current physical order (so in-group reordering is respected), keeps groups/ungrouped tabs in their relative order, and applies the **minimum** moves needed to pull a group's stray members back together — so an in-group reorder costs zero moves (no snap-back) and a native drag that splits a group self-heals. It's called (observer disconnected) after every mutation and in the observer's debounced callback. Chip placement in `_renderGroupChips` independently anchors to the physically-leftmost open member, not `tabIds[0]`. Don't move grouped tabs outside `_reflowTabs`, and don't reintroduce a fixed canonical order (it breaks in-group reordering).

**Manual overrides** (right-click "Move to group"/"New group…", or drag a tab onto a chip or onto any tab in the target group): recorded in `st.overrides` (`Map<tabId, groupKey>`) and take precedence over collection-based assignment everywhere. The sentinel value `UNGROUPED` means the user explicitly removed the tab from its group ("Remove from group"); auto-assign skips it so it isn't immediately pulled back into its collection's group. It's persisted (by itemID) and exempt from override pruning while its tab is open. Drag state is tracked via the module-level `_draggingTabId` (set on a delegated `dragstart` that only fires for reader/note tabs), not `dataTransfer` type checks, which are unreliable in Gecko during `dragover`. All reader tabs are made `draggable` in `_renderGroupChips`; the tab-region drop target is a delegated `dragover`/`drop` pair in `_setupTabBarObserver`.
