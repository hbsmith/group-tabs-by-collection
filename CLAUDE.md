...
**Drag-and-drop is REORDER-ONLY.** Dragging a tab can only reposition it *within its own region* — its own group, or the ungrouped tail — and never changes group membership; cross-region drops are ignored (snap-back). Membership changes go through the right-click menu (`_addTabToGroup` / `_removeTabFromGroup`) exclusively. This deliberately removed the membership-follows-drop behavior, which caused backward-drag jumps and splits. Custom HTML5 DnD handlers in `_setupTabBarObserver` run in the **capture phase with `stopPropagation` (including on `dragstart`/`dragenter`)** so Zotero's native tab drag never engages (otherwise the two fight: tabs jump and Zotero's frozen placeholder shows far from the cursor). `dragover` only `preventDefault`s — and shows the `.gtbc-insert-before/after` bar — for valid same-region targets. `_dropTab` rebuilds the full physical order and applies the minimum sequential `move(id, i)` calls (direction-agnostic, unlike a single computed destination index).

**Reordering whole groups:** chips are `draggable`; dragging a chip sets `_draggingGroupKey` and `_dropGroup` moves that group before/after the target group in `st.groups`, sets `st.manualOrder = true`, and calls `_layoutByGroupOrder` (library tab, then groups in `st.groups` order, then ungrouped). Once `manualOrder` is set, the `_compareGroups` sorts in `_groupNewTabs`/`_createGroupFromTab` are skipped (new groups append) and the order is persisted/restored. Because chips can be dragged, the collapse toggle fires on **click**, not `mousedown` (a `mousedown` `preventDefault` would block the drag); a real drag fires no click, so click cleanly means "tapped".

**Contiguity:** explicit ops (`_dropTab`, `_addTabToGroup`, `_autoAssignNewTabs`, `_applyGrouping`) position tabs so groups stay contiguous. `_healContiguity` re-heals a group that something *else* split — typically a newly opened tab Zotero inserted mid-group — and runs on the observer tick **only when `_draggingTabId` is null**. Reordering on the observer tick *during* a drag is what caused the "spazzing", so that guard is load-bearing: never reorder mid-drag. Chip placement in `_renderGroupChips` anchors to the physically-leftmost open member, not `tabIds[0]`.

**Manual overrides** (right-click "Move to group"/"New group…", or any drag): recorded in `st.overrides` (`Map<tabId, groupKey>`) and take precedence over collection-based assignment everywhere. The sentinel value `UNGROUPED` means the user explicitly removed the tab from its group (via "Remove from group", which also sends it to the end of the bar, or by dragging it to ungrouped space); auto-assign skips it so it isn't immediately pulled back into its collection's group. It's persisted (by itemID) and exempt from override pruning while its tab is open. Drag state is tracked via the module-level `_draggingTabId` (set on a delegated `dragstart` that only fires for reader/note tabs), not `dataTransfer` type checks, which are unreliable in Gecko during `dragover`. All reader tabs are made `draggable` in `_renderGroupChips`; the tab-region drop target is a delegated `dragover`/`drop` pair in `_setupTabBarObserver`.
